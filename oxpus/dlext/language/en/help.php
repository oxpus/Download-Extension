...
$lang = array_merge($lang, [
	...
	'HELP_DL_CAT_DESCRIPTION'	=> 'A short description for this category.<br />BBCodes are only available when the descriptions will alltimes be displayed on index.<br />This description will shown on the downloads index and on subcategories.',
	...
	'HELP_DL_DISABLE_NOTIFY'		=> 'With this Option you can enable or completely disable notifications about new added or edited downloads.<br />While this function is enabled here, it can individually be disabled while add or edit a download.<br />Only user will get an notification who have activate the notifications about new or edited downloads in their UCP.',
	...
	'HELP_DL_ENABLE_INDEX_DESC'		=> 'Hides the description of the downloads in the categories. <br /> If this option is deactivated, the length of the displayed description can be set with the following option.',
	...
	'HELP_DL_ENABLE_SEARCH_DESC'	=> 'Hides the description of the downloads in the search results. <br /> If this option is deactivated, the length of the displayed description can be set with the following option.',
	...
	'HELP_DL_GLOBAL_BOTS'		=> 'This option allow or deny the global access to the download area for bots.<br />All other permissions are not applied by this option.',
	'HELP_DL_GLOBAL_GUESTS'		=> 'This option allow or deny the global access to the download area for guests.<br />All other permissions are not applied by this option.',
	...
	'HELP_DL_LATEST_DOWNLOADS'		=> 'Determines whether this list is deactivated, shows all downloads (this corresponds to the overall view, sorted descending by age) or the actual last added or changed downloads.',
	...
	'HELP_DL_LIMIT_DESC_ON_SEARCH'	=> 'Cut the descriptions after the entered number of characters.<br />Set 0 here to disable this function.',
	...
	'HELP_DL_RSS_OFF_TEXT'				=> 'This text will be displayed instead of the download entries in the RSS feed if the function has been switched off and the previous option has been set to display this message accordingly. <br /> If forwarding has been set up in the previous option, this text will continue remain active but not displayed.',
	...
	'HELP_DL_RSS_PERMS'					=> 'Despite the selection of the categories from which entries should be displayed, it may be advisable to set the permissions of the user to log on or even more closely to that of guests or bots in order not to display downloads in the feed that the user should not be able to see . <br /> In the "for guests" setting, only the categories that a guest should be able to see are selected. <br /> Provided that the user or guest / bot is not shown any feeds due to the selected categories and the access rights set the feed behaves analogously to the settings as if it were switched off.',
	...
	'HELP_DL_RSS_NUMBER'				=> 'The maximum number of downloads in the feed.',
	'HELP_DL_RSS_SELECT'				=> 'This option determines whether the most recent or random downloads should be shown in the feed, depending on the selected categories, access rights and the number.',
	'HELP_DL_RSS_DESC_LENGTH'			=> 'With this option you can display the descriptions of the downloads or choose a shortened display (according to the setting for the download index). <br /> <br /> <strong> Attention: </strong> <br /> Since not every feed reader also recognizes and / or displays HTML codes, it can happen that the text is displayed incorrectly or the reader simply does not display any entries. In this case, the user must use a different reader or the descriptions would have to be switched off.',
	'HELP_DL_RSS_DESC_LENGTH_SHORTEN'	=> 'Cut the description of the downloads by x characters if the description is to be displayed in abbreviated form (see previous option). <br /> If 0 the description is not displayed!',

	'HELP_DL_SET_ADD'				=> 'With this option you can select with which user new downloads will be provided.<br />You can select the current user, an user selected by the category settings (even you have selected "category selection" here) or an other user which is registered in the forum.<br /><br />Please regard that the automatically generated download topic in the forum will still use the given user for thar function. This option will only change the "add user" setting for new downloads.<br /><br /><strong>Hint:</strong><br />The user id will not be checked by the download Extension itself, so a not existing ID can disrupt the functions!',
	...
	'HELP_DL_SHOW_FOOTER_STAT'		=> 'This option will turn the mini statistic on the download footer on and off.<br />The statistic will still earn data, if you switch it off.',
	...
	'HELP_DL_SIMILAR_DL_LIMIT'		=> 'Number of similar downloads which will be listed on the downloads details page.',
	...
	'HELP_DL_STAT_PERM'				=> 'Select here from which user level the download statistics can be viewed. <br /> For example, you can only activate moderators from download moderators, every board administrator and download moderator (NOT forum moderator!) Can open and view this page. <br /> Note that this page can take an extremely long time to load, so it is recommended that this page not open to many if you run a larger board and / or provide many downloads.',
	...
	'HELP_DL_TOPIC_TYPE'				=> 'This option chooses the topic type for the topics for the downloads.<br />After changing this type all new added or edited downloads will be posted with the new topic type. Existing topics will not be changed.',
	...
	'HELP_DL_VISUAL_CONFIRMATION'	=> 'Activate this option to allow users to enter a 5-digit confirmation code to allow the file to be downloaded. <br /> If the user has not entered a code or the code is incorrect, the extension will only display a message and not the download release. <br /> If this option is switched off, the user does not have to enter a code and can download the files directly from the details. ',
