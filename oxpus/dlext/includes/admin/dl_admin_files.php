...
{
	$dl_file = array();
	$dl_file = \oxpus\dlext\includes\classes\ dl_files::all_files(0, '', 'ASC', '', $df_id, 1, '*');
...
$index = array();
$index = \oxpus\dlext\includes\classes\ dl_main::full_index('', $cat_id);
...
if($action == 'edit' || $action == 'add')
{
	...
	if($action == 'edit')
	{
		...
		$tmp_ary				= \oxpus\dlext\includes\classes\ dl_format::dl_size($dl_extern_size, 2, 'select');
	...
	if (isset($index[$cat_id]['allow_thumbs']) && $index[$cat_id]['allow_thumbs'] && $config['dl_thumb_fsize'])
	{
		...
		$thumbnail_explain	= $language->lang('DL_THUMB_DIM_SIZE', $config['dl_thumb_xsize'], $config['dl_thumb_ysize'], \oxpus\dlext\includes\classes\ dl_format::dl_size($config['dl_thumb_fsize']));
...
else if($action == 'save')
{
	...
	else
	{
		...
		if ($df_id && !$file_extern)
		{
			...
			$index_new = array();
			$index_new = \oxpus\dlext\includes\classes\ dl_main::full_index('', $file_cat_old);
		...
		*/
		...
		if ($config['dl_enable_dl_topic'] && $approve)
		{
			if ($df_id)
			{
				\oxpus\dlext\includes\classes\ dl_topic::update_topic($dl_file['dl_topic'], $dl_t_id, $helper);
			}
			else
			{
				\oxpus\dlext\includes\classes\ dl_topic::gen_dl_topic($dl_t_id, $helper);
			}
		}
		...
		if ($approve)
		{
			$processing_user = \oxpus\dlext\includes\classes\ dl_auth::dl_auth_users($cat_id, 'auth_dl');
		...
		if (!$config['dl_disable_email'] && !$send_notify && $df_id && $sql_fav_user)
		{
			...
			\oxpus\dlext\includes\classes\ dl_email::send_dl_notify($mail_data, $helper, $ext_path);
		}
...
if ($action == '')
{
	...
	while ($row = $db->sql_fetchrow($result))
	{
		...
		if ($file_traffic)
		{
			$file_traffic = \oxpus\dlext\includes\classes\ dl_format::dl_size($file_traffic);
		}
		else
		{
		...
		if ($row['file_size'])
		{
			$file_size_kb	= \oxpus\dlext\includes\classes\ dl_format::dl_size($row['file_size']);
		}
		else
		{
	...
	$categories .= \oxpus\dlext\includes\classes\ dl_extra::dl_dropdown(0, 0, $cat_id, 'auth_up');
	$categories .= '</select>';
