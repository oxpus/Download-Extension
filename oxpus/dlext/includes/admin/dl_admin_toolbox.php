...
if (isset($index) && sizeof($index))
{
	...
	{
		$read_files = \oxpus\dlext\includes\classes\ dl_physical::read_dl_files('', $unas_files);
	...
	if (sizeof($real_thumbnails['file_name']))
	{
		...
		for ($i = 0; $i < sizeof($real_thumbnails['file_name']); $i++)
		{
			...
			$template->assign_block_vars('thumbnails', array(
				'CHECKBOX'		=> $checkbox,
				'REAL_FILE'		=> $real_file,
				'FILE_SIZE'		=> \oxpus\dlext\includes\classes\ dl_format::dl_size($real_thumbnails['file_size'][$i]),
...
if ($action == 'browse' || $action == '' || $action == 'unassigned')
{
	if ($action != 'unassigned')
	{
		...
		$existing_files = \oxpus\dlext\includes\classes\ dl_physical::read_exist_files();
	...
	if ($files)
	{
		...
		foreach($files as $i => $value)
		{
			...
			$file_size_tmp = \oxpus\dlext\includes\classes\ dl_format::dl_size($file_size, 2, 'no');
		...
		$overall_size_tmp = \oxpus\dlext\includes\classes\ dl_format::dl_size($overall_size, 2, 'no');
		...
		if ($action != 'unassigned')
		{
			...
			$s_file_action .= \oxpus\dlext\includes\classes\ dl_physical::read_dl_dirs();
		}
		else
		{
			...
			$s_file_action .= '<option value="del">'.$language->lang('DL_DELETE').'</option>';
			$s_file_action .= '<option value="---">---------------</option>';
			$s_file_action .= \oxpus\dlext\includes\classes\ dl_extra::dl_dropdown(0, 0, 0, 'auth_view');
		}
