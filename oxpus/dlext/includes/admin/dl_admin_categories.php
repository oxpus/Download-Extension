...
if($action == 'edit' || $action == 'add')
{
	...
	if($action == 'edit' && $cat_id && !$submit)
	{
		...
		$s_cat_parent		.= \oxpus\dlext\includes\classes\ dl_extra::dl_dropdown(0, 0, $index[$cat_id]['parent'], 'auth_view', $cat_id);
		$s_cat_parent		.= '</select>';
		...
		$perms_copy_from	.= \oxpus\dlext\includes\classes\ dl_extra::dl_dropdown(0, 0, $index[$cat_id]['parent'], 'auth_view', $cat_id);
	...
	else
	{
		...
		$s_cat_parent		= '<select name="parent">';
		$s_cat_parent		.= '<option value="0">&nbsp;»&nbsp;'.$language->lang('DL_CAT_INDEX').'</option>';
		$s_cat_parent		.= \oxpus\dlext\includes\classes\ dl_extra::dl_dropdown(0, 0, $cat_parent_id, 'auth_view', -1);
		$s_cat_parent		.= '</select>';
		$cat_remain_traffic	= $cat_traffic;
		$perm_cat_id		= $perms_copy_from;
		$perms_copy_from	= '<select name="perms_copy_from">';
		$perms_copy_from	.= '<option value="0">&nbsp;»&nbsp;'.$language->lang('DL_CAT_PARENT').'</option>';
		$perms_copy_from	.= \oxpus\dlext\includes\classes\ dl_extra::dl_dropdown(0, 0, $perm_cat_id, 'auth_view', -1);
		$perms_copy_from	.= '</select>';
	}

	$t_path_select = \oxpus\dlext\includes\classes\ dl_physical::get_file_base_tree(DL_EXT_FILES_WEBFOLDER, $cat_path);
	...
	$cat_remain_traffic	= \oxpus\dlext\includes\classes\ dl_format::dl_size($cat_remain_traffic);
...
else if($action == 'delete' && $cat_id && !\oxpus\dlext\includes\classes\ dl_main::get_sublevel_count($cat_id))
{
	if ($confirm)
	{
		if (!check_form_key('dl_adm_cats'))
	...
	else
	{
		...
		$s_switch_cat .= \oxpus\dlext\includes\classes\ dl_extra::dl_dropdown(0, 0, $cat_id, 'auth_move');
		$s_switch_cat .= '</select>';
...
if (!$dl_template_in_use)
{
	...
	foreach (array_keys($index) as $key)
	{
		...
		if (($idx_type == 'c' && $index[$cat_id]['parent'] == $cat_parent) || $idx_type == 'f')
		{
			...
			$cat_sub = \oxpus\dlext\includes\classes\ dl_main::get_sublevel_count($cat_id);
			$cat_sub_count = \oxpus\dlext\includes\classes\ dl_main::count_sublevel($cat_id);
	...
	{
		$tmp_nav = array();
		$cat_navi = \oxpus\dlext\includes\classes\ dl_nav::nav($helper, $cat_parent, 'acp', $tmp_nav, $basic_link_open);
	}
