...
$bug_access_cats = \oxpus\dlext\includes\classes\ dl_main::full_index($this->helper, 0, 0, 0, 1);
...
if ($preview && $this->user->data['is_registered'])
{
	...
	$action = ($fav_id && \oxpus\dlext\includes\classes\ dl_auth::user_admin()) ? 'edit' : 'add';
}
...
if ($action == 'save' && $this->user->data['is_registered'])
{
	...
	if ($fav_id && \oxpus\dlext\includes\classes\ dl_auth::user_admin())
	{
...
if ($action == 'status' && $fav_id && $allow_bug_mod)
{
	...
	if ($report_author_id <> $this->user->data['user_id'])
	{
		...
		\oxpus\dlext\includes\classes\ dl_email::send_bt_status($mail_data, $this->helper, $ext_path);
	}
...
if ($action == 'assign' && $df_id && $fav_id && $allow_bug_mod)
{
	...
	if ($new_user_id <> $this->user->data['user_id'])
	{
		...
		\oxpus\dlext\includes\classes\ dl_email::send_bt_assign($mail_data, $this->helper, $ext_path);
	}
...
*/
if (($action == 'add' && $this->user->data['is_registered']) || ($action == 'edit' && \oxpus\dlext\includes\classes\ dl_auth::user_admin() && $fav_id))
{
	$this->template->set_filenames(array(
		'body' => 'dl_bt_add.html')
	);
