...
if ($this->config['dl_rss_enable'])
{
	...
	*/
	...
	if (sizeof($access_cats))
	{
		...
		$dl_files = array();
		$dl_files = \oxpus\dlext\includes\classes\ dl_files::all_files(0, $sql_sort_by, $sql_order_by , $sql_where_cats, 0, 0, $sql_fields, $sql_limit);
		...
		if (sizeof($dl_files))
		{
			...
			for ($i = 0; $i < sizeof($dl_files); $i++)
			{
				...
				{
					$mini_status = \oxpus\dlext\includes\classes\ dl_status::mini_status_file($dl_cat, $dl_id, $rss);
				}
				else
				{
					$mini_status = '';
