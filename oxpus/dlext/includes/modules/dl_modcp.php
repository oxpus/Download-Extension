...
{
	$access_cat = array();
	$access_cat = \oxpus\dlext\includes\classes\ dl_main::full_index($this->helper, 0, 0, 0, 2);
}
		...
		*/
		if ($action == 'save' && $submit)
		{
			...
			else
			{
				...
				{
					$file_traffic = \oxpus\dlext\includes\classes\ dl_format::resize_value('dl_file_traffic', $file_traffic);
				}

				$dl_file = array();
				$dl_file = \oxpus\dlext\includes\classes\ dl_files::all_files(0, 0, 'ASC', 0, $df_id, true, '*');
				...
				$ext_blacklist = \oxpus\dlext\includes\classes\ dl_auth::get_ext_blacklist();
				...
				if (!$file_extern)
				{
					...
					if ($file_name)
					{
						...
						if ($index[$cat_id]['statistics'])
						{
							...
							{
								$stat_prune = \oxpus\dlext\includes\classes\ dl_main::dl_prune_stats($cat_id, $index[$cat_id]['stats_prune']);
							}

							$browser = \oxpus\dlext\includes\classes\ dl_init::dl_client($this->user->data['session_browser'], $ext_path);
				...
				if ($file_extern)
				{
					$file_size = \oxpus\dlext\includes\classes\ dl_format::resize_value('dl_extern_size', $file_extern_size);
				}
				...
				if ($df_id && $new_cat)
				{
					...
					*/
					...
					\oxpus\dlext\includes\classes\ dl_topic::update_topic($dl_file['dl_topic'], $df_id, $this->helper);

					if ($approve)
					{
						$processing_user	= \oxpus\dlext\includes\classes\ dl_auth::dl_auth_users($cat_id, 'auth_view');
						$email_template		= 'downloads_change_notify';

						\oxpus\dlext\includes\classes\ dl_topic::gen_dl_topic($df_id, $this->helper);
					}
					else
					{
					...
					if (!$this->config['dl_disable_email'] && !$send_notify && $sql_fav_user)
					{
						...
						\oxpus\dlext\includes\classes\ dl_email::send_dl_notify($mail_data, $this->helper, $ext_path);
					}
		...
		if ($action == 'delete')
		{
			...
			if (!empty($dl_id))
			{
				if (!$confirm)
				{
					...
					{
						$dl_file	= array();
						$dl_file	= \oxpus\dlext\includes\classes\ dl_files::all_files(0, '', 'ASC', '', intval($dl_id[0]), true, '*');
		...
		{
			$dl_file = array();
			$dl_file = \oxpus\dlext\includes\classes\ dl_files::all_files(0, '', 'ASC', '', $df_id, true, '*');
			...
			if ($index[$cat_id]['allow_thumbs'] && $this->config['dl_thumb_fsize'])
			{
				...
				$thumbnail_explain	= $this->language->lang('DL_THUMB_DIM_SIZE', $this->config['dl_thumb_xsize'], $this->config['dl_thumb_ysize'], \oxpus\dlext\includes\classes\ dl_format::dl_size($this->config['dl_thumb_fsize']));
			...
			$tmp_ary				= \oxpus\dlext\includes\classes\ dl_format::dl_size($file_traffic, 2, 'select');
			$file_traffic_out		= $tmp_ary['size_out'];
			$data_range_select		= $tmp_ary['range'];

			$tmp_ary				= \oxpus\dlext\includes\classes\ dl_format::dl_size($file_size, 2, 'select');
			...
			if (!$own_edit)
			{
				...
				$select_code .= \oxpus\dlext\includes\classes\ dl_extra::dl_dropdown(0, 0, $cat_id, 'auth_up');
				$select_code .= '</select>';
			...
			$this->template->assign_vars(array(
				...
				'MAX_UPLOAD_SIZE' => $this->language->lang('DL_UPLOAD_MAX_FILESIZE', \oxpus\dlext\includes\classes\ dl_physical::dl_max_upload_size()),
		...
		if ($action == 'approve')
		{
			if (!empty($dl_id))
			{
				...
				if ($this->config['dl_enable_dl_topic'])
				{
					for ($i = 0; $i < sizeof($dl_id); $i++)
					{
						\oxpus\dlext\includes\classes\ dl_topic::gen_dl_topic(intval($dl_id[$i]), $this->helper);
					}
				}
		...
		if ($action == 'manage' && $cat_id)
		{
			...
			while ($row = $this->db->sql_fetchrow($result))
			{
				...
				$mini_icon		= \oxpus\dlext\includes\classes\ dl_status::mini_status_file($cat_id, $file_id);
			...
			$s_cat_select .= \oxpus\dlext\includes\classes\ dl_extra::dl_dropdown(0, 0, $cat_id, 'auth_view');
			$s_cat_select .= '</select>';
