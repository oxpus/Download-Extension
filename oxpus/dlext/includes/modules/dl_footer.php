...
if (sizeof($index) || $cat)
{
	...
	*/
	$total_approve = \oxpus\dlext\includes\classes\ dl_counter::count_dl_approve();
	if ($total_approve)
	{
	...
	*/
	$total_comment_approve = \oxpus\dlext\includes\classes\ dl_counter::count_comments_approve();
	...
	*/
	$stats_view = \oxpus\dlext\includes\classes\ dl_auth::stats_perm();
	if ($stats_view)
	{
	...
	*/
	if ($this->config['dl_enable_jumpbox'])
	{
		...
		$dl_jumpbox .= \oxpus\dlext\includes\classes\ dl_extra::dl_dropdown(0, 0, $cat, 'auth_view');
	...
	if ($ext_stats_enable)
	{
		$overall_traffic = \oxpus\dlext\includes\classes\ dl_format::dl_size($this->config['dl_overall_traffic']);
		$overall_guest_traffic = \oxpus\dlext\includes\classes\ dl_format::dl_size($this->config['dl_overall_guest_traffic']);
	...
	*/
	...
	$dl_latest_files = \oxpus\dlext\includes\classes\ dl_files::all_files(0, '', '', $sql_latest_where, 0, 0, 'id');
	...
	*/
	...
	if ($this->config['dl_show_footer_stat'] && !$this->config['dl_traffic_off'])
	{
		...
		if ($this->user->data['is_registered'] && DL_OVERALL_TRAFFICS == true)
		{
			...
			{
				$remain_text_out = $this->language->lang('DL_REMAIN_OVERALL_TRAFFIC') . '<b>' . \oxpus\dlext\includes\classes\ dl_format::dl_size($remain_traffic, 2) . '</b>';
		...
		if ($this->user->data['is_registered'] && DL_USERS_TRAFFICS == true)
		{
			...
			$user_traffic_out		= \oxpus\dlext\includes\classes\ dl_format::dl_size($user_traffic, 2);
		...
		if ((!$this->user->data['is_registered'] || $this->user->data['user_type'] == USER_FOUNDER) && DL_GUESTS_TRAFFICS == true)
		{
			...
			else
			{
				...
				$remain_guest_text_out	= $this->language->lang('DL_REMAIN_OVERALL_GUEST_TRAFFIC') . '<b>' . \oxpus\dlext\includes\classes\ dl_format::dl_size($remain_guest_traffic, 2) . '</b>';
