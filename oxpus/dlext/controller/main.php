	...
	*/
	...
	public function handle($view = '')
	{
		...
		*/
		...
		for ($i = 0; $i < sizeof($nav_string['link']); $i++)
		{
			$this->template->assign_block_vars('navlinks', array(
				'U_VIEW_FORUM'	=> $this->helper->route('oxpus_dlext_controller', $nav_string['link'][$i]),
				...
				if (isset($dl_todo['file_name'][0]) && sizeof($dl_todo['file_name']))
				{
					for ($i = 0; $i < sizeof($dl_todo['file_name']); $i++)
					{
						$this->template->assign_block_vars('todolist_row', array(
							...
							'FILE_LINK'		=> $this->helper->route('oxpus_dlext_controller', array('view' => 'detail', 'df_id' => $dl_todo['df_id'][$i])),
							'HACK_VERSION'	=> $dl_todo['hack_version'][$i],
							'TODO'			=> $dl_todo['todo'][$i],

							'U_TODO_EDIT'	=> $this->helper->route('oxpus_dlext_controller', array('view' => 'todo', 'action' => 'edit', 'edit' => true, 'df_id' => $dl_todo['df_id'][$i])),
							'U_TODO_DELETE'	=> $this->helper->route('oxpus_dlext_controller', array('view' => 'todo', 'action' => 'edit', 'delete' => true, 'submit' => true, 'df_id' => $dl_todo['df_id'][$i])),
						));
					}
				}
				else
		...
		*/
		...
		$this->template->assign_vars(array(
			'U_HELP_POPUP' => $this->helper->route('oxpus_dlext_controller', array('view' => 'help')),
		));
