		...
		*/
		...
		while ($row = $this->db->sql_fetchrow($result))
		{
			if ($row['cat'] != $last_id)
			{
				...
				$last_dl[$last_id]['link'] = $this->helper->route('oxpus_dlext_details', array('df_id' => $row['id']));
		...
		if (sizeof($index) > 0)
		{
			foreach(array_keys($index) as $cat_id)
			{
				...
				$last_dl_time = $this->dlext_main->find_latest_dl($last_dl, $cat_id, $cat_id, array());
				...
				if ($cat_subs)
				{
					...
					for ($j = 0; $j < sizeof($cat_subs); $j++)
					{
						...
						$this->template->assign_block_vars($block.'.sub.sublevel_row', array(
							...
							'SUBLEVEL_COUNT' => $cat_sublevel['total'][$j] + $this->dlext_main->get_sublevel_count($cat_sublevel['cat_id'][$j]),
		...
		{
			$dl_files = array();
			$dl_files = $this->dlext_files->files($cat, $sql_sort_by, $sql_order, $start, $this->config['dl_links_per_page'], 'id, description, desc_uid, desc_bitfield, desc_flags, hack_version, extern, file_size, klicks, overall_klicks, rating, long_desc, long_desc_uid, long_desc_bitfield, long_desc_flags, add_user');
			...
			for ($i = 0; $i < sizeof($dl_files); $i++)
			{
				...
				$file_url = $this->helper->route('oxpus_dlext_details', array('df_id' => $file_id));
				...
				{
					$this->template->assign_block_vars('downloads.comments', array('U_COMMENT' => $this->helper->route('oxpus_dlext_details', array('view' => 'comment', 'action' => 'view', 'cat_id' => $cat, 'df_id' => $file_id))));
				}
			}
		}
		...
		$this->template->assign_vars(array(
			...
			'DL_UPLOAD'		=> $this->helper->route('oxpus_dlext_upload', array('cat_id' => $cat)),
			'PHPEX'			=> $this->php_ext,
			...
			'U_DOWNLOADS'	=> ($cat) ? $this->helper->route('oxpus_dlext_index', array('cat' => $cat)) : $this->helper->route('oxpus_dlext_index'),
