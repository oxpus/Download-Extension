...
class mcp_manage
{
	...
	*/
	...
	public function handle()
	{
		...
		if (isset($index[$cat_id]['total']) && $index[$cat_id]['total'] && $cat_id && !$deny_modcp)
		{
			...
			while ($row = $this->db->sql_fetchrow($result))
			{
				...
				$this->template->assign_block_vars('manage_row', array(
					...
					'U_UP'			=> ($this->auth->acl_get('a_') && $this->user->data['is_registered']) ? $this->helper->route('oxpus_dlext_mcp_manage', array('fmove' => -1, 'sort' => 1, 'df_id' => $file_id, 'cat_id' => $cat_id)) : '',
					'U_DOWN'		=> ($this->auth->acl_get('a_') && $this->user->data['is_registered']) ? $this->helper->route('oxpus_dlext_mcp_manage', array('fmove' => 1, 'sort' => 1, 'df_id' => $file_id, 'cat_id' => $cat_id)) : '',
			...
			if ($total_downloads > $per_page)
			{
				...
				$pagination->generate_template_pagination(
					$this->helper->route('oxpus_dlext_mcp_manage', array('cat_id' => $cat_id)),
					'pagination',
					'start',
					$total_downloads,
					$per_page,
			...
			$this->template->assign_vars(array(
				...
				'U_SORT_ASC'		=> ($this->auth->acl_get('a_') && $this->user->data['is_registered']) ? $this->helper->route('oxpus_dlext_mcp_manage', array('fmove' => 'ABC', 'sort' => (($sort) ? 1 : ''), 'df_id' => $file_id,  'cat_id' => $cat_id)) : '',
				'S_CAT_SELECT'		=> $s_cat_select,
				'S_DL_MODCP_ACTION'	=> $this->helper->route('oxpus_dlext_mcp_manage'),
		...
		if (!$deny_modcp)
		{
			$s_cat_select = '<form method="post" id="mcp_cat_select" action="' . $this->helper->route('oxpus_dlext_mcp_manage') . '" onsubmit="if(this.options[this.selectedIndex].value == -1) { return false; }">';
