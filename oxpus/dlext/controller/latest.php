	...
	*/
	...
	public function handle()
	{
		...
		*/
		...
		extract($this->dispatcher->trigger_event('oxpus.dlext.latest_fetch_download_data', compact($vars)));
			...
			for ($i = 0; $i < count($dl_files); ++$i)
			{
				...
				if (isset($cat_auth['auth_view']) && $cat_auth['auth_view'] || isset($index[$cat_id]['auth_view']) && $index[$cat_id]['auth_view'] || $this->dlext_auth->user_admin())
				{
					...
					*/
					...
					extract($this->dispatcher->trigger_event('oxpus.dlext.latest_display_data_after', compact($vars)));
				}
			}
		...
		*/
		$this->dlext_footer->set_parameter('latest', 0, 0, $index);
		$this->dlext_footer->handle();

		/*
		* generate page
		*/
		return $this->helper->render('@oxpus_dlext/dl_latest_body.html', $this->language->lang('DL_LATEST_DOWNLOADS'));
	}
}
