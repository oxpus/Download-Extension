	...
	public static function send_dl_notify($mail_data, $helper, $ext_path)
	{
		...
		while ($row = $db->sql_fetchrow($result))
		{
			...
			$messenger->assign_vars(array(
				...
				'U_APPROVE'		=> generate_board_url(true) . $helper->route('dl_ext_controller', array('view' => 'modcp', 'action' => 'approve'), false, ''),
				'U_CATEGORY'	=> generate_board_url(true) . $helper->route('dl_ext_controller', array('cat' => $cat_id), false, ''),
			));
	...
	public static function send_comment_notify($mail_data, $helper, $ext_path)
	{
		...
		while ($row = $db->sql_fetchrow($result))
		{
			...
			$messenger->assign_vars(array(
				...
				'USERNAME'		=> htmlspecialchars_decode($row['username']),
				'DOWNLOAD'		=> htmlspecialchars_decode($mail_data['description']),
				'U_APPROVE'		=> generate_board_url(true) . $helper->route('dl_ext_controller', array('view' => 'modcp', 'action' => 'capprove'), false, ''),
				'U_DOWNLOAD'	=> generate_board_url(true) . $helper->route('dl_ext_controller', array('view' => 'comment', 'action' => 'view', 'cat_id' => $cat_id, 'df_id' => $df_id), false, ''),
			));
			$messenger->send(NOTIFY_EMAIL);
		}
	...
	public static function send_report($mail_data, $helper, $ext_path)
	{
		...
		while ($row = $db->sql_fetchrow($result))
		{
			...
			$messenger->assign_vars(array(
				...
				'USERNAME'				=> htmlspecialchars_decode($row['username']),
				'U_DOWNLOAD'			=> generate_board_url(true) . $helper->route('dl_ext_controller', array('view' => 'detail', 'cat_id' => $cat_id, 'df_id' => $df_id), false, ''),
			));
