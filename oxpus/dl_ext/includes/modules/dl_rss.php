...
if ($this->config['dl_rss_enable'])
{
	...
	*/
	...
	if (sizeof($access_cats))
	{
		...
		$dl_files = array();
		$dl_files = \oxpus\dl_ext\includes\classes\ dl_files::all_files(0, $sql_sort_by, $sql_order_by , $sql_where_cats, 0, 0, $sql_fields, $sql_limit);
		...
		if (sizeof($dl_files))
		{
			...
			for ($i = 0; $i < sizeof($dl_files); $i++)
			{
				...
				$this->template->assign_block_vars('dl_rss_feed', array(
					...
					'U_DL_RSS'		=> generate_board_url(true) . $this->helper->route('dl_ext_controller', array('view' => 'detail', 'df_id' => $dl_id)),
				));
			}
