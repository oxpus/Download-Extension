...
{
	$access_cats = array();
	$access_cats = \oxpus\dl_ext\includes\classes\ dl_main::full_index($this->helper, 0, 0, 0, 1);
		...
		*/
		...
		if ($total_top_ten)
		{
			...
			while ($row = $this->db->sql_fetchrow($result))
			{
				...
				$file_link		= $this->helper->route('dl_ext_controller', array('view' => 'detail', 'df_id' => $file_id));
			...
			*/
			...
			if ($total_top_ten)
			{
				...
				while ($row = $this->db->sql_fetchrow($result))
				{
					...
					$description	= $row['description'];
					$cat_name		= $row['cat_name'];
					$dl_traffic		= \oxpus\dl_ext\includes\classes\ dl_format::dl_size($row['month_traffic']);
			...
			*/
			...
			if ($total_top_ten)
			{
				...
				while ($row = $this->db->sql_fetchrow($result))
				{
					...
					$description	= $row['description'];
					$cat_name		= $row['cat_name'];
					$dl_traffic		= \oxpus\dl_ext\includes\classes\ dl_format::dl_size($row['overall_traffic']);
			...
			*/
			...
			if ($total_top_ten)
			{
				...
				while ($row = $this->db->sql_fetchrow($result))
				{
					...
					$dl_traffic	= \oxpus\dl_ext\includes\classes\ dl_format::dl_size($row['dl_traffic']);
