...
$bug_access_cats = \oxpus\dl_ext\includes\classes\ dl_main::full_index($this->helper, 0, 0, 0, 1);
...
if ($preview && $this->user->data['is_registered'])
{
	...
	$action = ($fav_id && \oxpus\dl_ext\includes\classes\ dl_auth::user_admin()) ? 'edit' : 'add';
}
...
if ($action == 'save' && $this->user->data['is_registered'])
{
	...
	if ($fav_id && \oxpus\dl_ext\includes\classes\ dl_auth::user_admin())
	{
	...
	$message = $this->language->lang('DL_BUG_REPORT_ADDED') . '<br /><br />' . $this->language->lang('CLICK_RETURN_BUG_TRACKER', '<a href="' . $this->helper->route('dl_ext_controller', $link_array) . '">', '</a>');
...
if ($action == 'status' && $fav_id && $allow_bug_mod)
{
	...
	if ($report_author_id <> $this->user->data['user_id'])
	{
		...
		\oxpus\dl_ext\includes\classes\ dl_email::send_bt_status($mail_data, $this->helper, $ext_path);
	}
...
if ($action == 'assign' && $df_id && $fav_id && $allow_bug_mod)
{
	...
	if ($new_user_id <> $this->user->data['user_id'])
	{
		...
		\oxpus\dl_ext\includes\classes\ dl_email::send_bt_assign($mail_data, $this->helper, $ext_path);
	}
...
if ($action == 'detail' && $fav_id)
{
	...
	$u_report_file_link		= $this->helper->route('dl_ext_controller', array('view' => 'detail', 'df_id' => $report_file_id));
	...
	if ($allow_bug_mod)
	{
		$this->template->assign_block_vars('delete', array(
			'U_DELETE' => $this->helper->route('dl_ext_controller', array('view' => 'bug_tracker', 'df_id' => $report_file_id, 'fav_id' => $report_id, 'action' => 'delete')),
		));
		...
		if ($report_status < 4)
		{
			...
			if ($this->db->sql_affectedrows($result))
			{
				...
				$this->template->assign_vars(array(
					'S_FORM_ASSIGN_ACTION' => $this->helper->route('dl_ext_controller', array('view' => 'bug_tracker', 'action' => 'assign', 'df_id' => $report_file_id, 'fav_id' => $fav_id)),
		...
		if ($s_select_status)
		{
			...
			$this->template->assign_vars(array(
				'S_FORM_STATUS_ACTION' => $this->helper->route('dl_ext_controller', array('view' => 'bug_tracker', 'action' => 'status', 'df_id' => $report_file_id, 'fav_id' => $fav_id)),
				'S_SELECT_STATUS' => $s_select_status)
			);
		}
	}

	if (\oxpus\dl_ext\includes\classes\ dl_auth::user_admin())
	{
		$this->template->assign_vars(array(
			'I_BUG_REPORT'		=> $this->user->img('icon_post_edit', 'EDIT_POST'),
			'U_BUG_REPORT_EDIT'	=> $this->helper->route('dl_ext_controller', array('view' => 'bug_tracker', 'action' => 'edit', 'fav_id' => $fav_id)),
		));
	}
...
*/
if (($action == 'add' && $this->user->data['is_registered']) || ($action == 'edit' && \oxpus\dl_ext\includes\classes\ dl_auth::user_admin() && $fav_id))
{
	$this->template->set_filenames(array(
		'body' => 'dl_bt_add.html')
	);
	...
	if ($reports_num)
	{
		while ($row = $this->db->sql_fetchrow($result))
		{
			...
			$this->template->assign_block_vars('bug_tracker_row', array(
				...
				'REPORT_FILE'			=> $report_file,
				'REPORT_FILE_VER'		=> $report_file_ver,
				'REPORT_FILE_LINK'		=> $this->helper->route('dl_ext_controller', array('view' => 'detail', 'df_id' => $report_dl_id)),
				...
				'REPORT_DETAIL'			=> $this->helper->route('dl_ext_controller', array('view' => 'bug_tracker', 'fav_id' => $report_id, 'action' => 'detail')),
			));
			...
			if ($allow_bug_mod)
			{
				$this->template->assign_block_vars('bug_tracker_row.modext', array(
					'U_DELETE' => $this->helper->route('dl_ext_controller', array('view' => 'bug_tracker', 'df_id' => $report_dl_id, 'fav_id' => $report_id, 'action' => 'delete')),
				));

				$this->template->assign_block_vars('bug_tracker_row.status_mod', array(
					'U_STATUS' => $this->helper->route('dl_ext_controller', array('view' => 'bug_tracker', 'df_id' => $report_dl_id, 'fav_id' => $report_id, 'action' => 'status')),
				));
			}
			else
			{
