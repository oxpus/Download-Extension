...
if ($submit)
{
	...
	*/
	...
	$message = $this->language->lang('DL_USER_CONFIG_SAVED', '<a href="' . $this->helper->route('dl_ext_controller', array('view' => 'user_config')) . '">', '</a>');
...
*/
$access_cat = array();
$access_cat = \oxpus\dl_ext\includes\classes\ dl_main::full_index($this->helper, 0, 0, 0, 1);
...
if ($total_favorites)
{
	...
	while ($row = $this->db->sql_fetchrow($result))
	{
		...
		$dl_nav		= \oxpus\dl_ext\includes\classes\ dl_nav::nav($this->helper, $row['cat'], 'links', $tmp_nav) . '&nbsp;»&nbsp;';

		$this->template->assign_block_vars('favorite_row', array(
			'DL_ID'			=> $row['fav_id'],
			'DL_CAT'		=> $dl_nav,
			'DOWNLOAD'		=> $row['description'],
			'U_DOWNLOAD'	=> $this->helper->route('dl_ext_controller', array('view' => 'detail', 'df_id' => $row['id'], 'cat_id' => $row['cat'])),
		));
	}
}
...
$this->template->assign_vars(array(
	...
	'S_FORM_ACTION'						=> $this->helper->route('dl_ext_controller', array('view' => 'user_config')),
));
