...
if ($submit)
{
	...
	if($cat_id)
	{
		...
		if ($approve)
		{
			...
			\oxpus\dl_ext\includes\classes\ dl_topic::gen_dl_topic($next_id, $this->helper);
		}
		else
		{
		...
		$message = $this->language->lang('DOWNLOAD_ADDED') . $thumb_message . $approve_message . '<br /><br />' . $this->language->lang('CLICK_RETURN_DOWNLOADS', '<a href="' . $this->helper->route('dl_ext_controller', array('cat' => $cat_id)) . '">', '</a>');
		if ($cat_auth['auth_up'])
		{
...
$s_cat_select .= \oxpus\dl_ext\includes\classes\ dl_extra::dl_dropdown(0, 0, $cat_id, 'auth_up');
$s_cat_select .= '</select>';
...
$file_size_ary		= \oxpus\dl_ext\includes\classes\ dl_format::dl_size(0, 2, 'select');
