	...
	*/
	...
	if ($this->config['dl_show_footer_stat'] && !$this->config['dl_traffic_off'])
	{
		...
		if ($this->user->data['is_registered'] && DL_OVERALL_TRAFFICS == true)
		{
			...
			{
				$remain_text_out = $this->language->lang('DL_REMAIN_OVERALL_TRAFFIC') . '<b>' . \oxpus\dl_ext\includes\classes\ dl_format::dl_size($remain_traffic, 2) . '</b>';
		...
		if ((!$this->user->data['is_registered'] || $this->user->data['user_type'] == USER_FOUNDER) && DL_GUESTS_TRAFFICS == true)
		{
			...
			else
			{
				...
				$remain_guest_text_out	= $this->language->lang('DL_REMAIN_OVERALL_GUEST_TRAFFIC') . '<b>' . \oxpus\dl_ext\includes\classes\ dl_format::dl_size($remain_guest_traffic, 2) . '</b>';
