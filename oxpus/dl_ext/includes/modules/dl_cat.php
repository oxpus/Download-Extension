...
{
	$cat_auth = array();
	$cat_auth = \oxpus\dl_ext\includes\classes\ dl_auth::dl_cat_auth($cat);
	$index_auth = array();
	$index_auth = \oxpus\dl_ext\includes\classes\ dl_main::full_index($this->helper, $cat);
...
while ($row = $this->db->sql_fetchrow($result))
{
	if ($row['cat'] != $last_id)
	{
		...
		$last_dl[$last_id]['link'] = $this->helper->route('dl_ext_controller', array('view' => 'detail', 'df_id' => $row['id']));
...
if (sizeof($index) > 0)
{
	foreach(array_keys($index) as $cat_id)
	{
		...
		$mini_icon = array();
		$mini_icon = \oxpus\dl_ext\includes\classes\ dl_status::mini_status_cat($cat_id, $cat_id);
		...
		if ($cat_subs)
		{
			...
			for ($j = 0; $j < sizeof($cat_subs); $j++)
			{
				...
				$mini_icon = array();
				$mini_icon = \oxpus\dl_ext\includes\classes\ dl_status::mini_status_cat($sub_id, $sub_id);
...
{
	$index_cat = array();
	$index_cat = \oxpus\dl_ext\includes\classes\ dl_main::full_index($this->helper, $cat);
	...
	if (\oxpus\dl_ext\includes\classes\ dl_auth::user_auth($cat, 'auth_mod'))
	{
...
{
	$dl_files = array();
	$dl_files = \oxpus\dl_ext\includes\classes\ dl_files::files($cat, $sql_sort_by, $sql_order, $start, $this->config['dl_links_per_page'], 'id, description, desc_uid, desc_bitfield, desc_flags, hack_version, extern, file_size, klicks, overall_klicks, rating, long_desc, long_desc_uid, long_desc_bitfield, long_desc_flags, add_user');

	if (\oxpus\dl_ext\includes\classes\ dl_auth::cat_auth_comment_read($cat))
	{
	...
	for ($i = 0; $i < sizeof($dl_files); $i++)
	{
		...
		{
			$file_size = \oxpus\dl_ext\includes\classes\ dl_format::dl_size($dl_files[$i]['file_size'], 2);
		}
		else
		{
		...
		$this->template->assign_block_vars('downloads', array(
			...
			'DF_ID'					=> $file_id,
			'U_DIRECT_EDIT'			=> ($cat_edit_link) ? $this->helper->route('dl_ext_controller', array('view' => 'modcp', 'action' => 'edit', 'cat_id' => $cat, 'df_id' => $file_id)) : '',
			'U_FILE'				=> $file_url)
		);
...
$this->template->assign_vars(array(
	...
	'DL_MODCP'		=> (isset($total_downloads) && $total_downloads <> 0 && \oxpus\dl_ext\includes\classes\ dl_auth::user_auth($cat, 'auth_mod')) ? $this->language->lang('DL_MODCP_MOD_AUTH', '<a href="' . $this->helper->route('dl_ext_controller', array('view' => 'modcp', 'cat_id' => $cat)) . '">', '</a>') : '',
