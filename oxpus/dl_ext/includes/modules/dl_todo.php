...
if ($submit && !$cancel)
{
	...
	if ($df_id)
	{
		...
		$meta_url	= $this->helper->route('dl_ext_controller', array('view' => 'todo', 'action' => 'edit'));
...
{
	$dl_file = array();
	$dl_file = \oxpus\dl_ext\includes\classes\ dl_files::all_files(0, '', 'ASC', '', $df_id, 0, 'description, desc_uid, desc_flags, todo, todo_uid, todo_flags');
...
if (isset($dl_todo['file_name'][0]))
{
	for ($i = 0; $i < sizeof($dl_todo['file_name']); $i++)
	{
		...
		$this->template->assign_block_vars('todolist_row', array(
			...
			'FILE_LINK'		=> $this->helper->route('dl_ext_controller', array('view' => 'detail', 'df_id' => $df_id)),
			'HACK_VERSION'	=> $dl_todo['hack_version'][$i],
			'TODO'			=> $dl_todo['todo'][$i],

			'U_TODO_EDIT'	=> $this->helper->route('dl_ext_controller', array('view' => 'todo', 'action' => 'edit', 'edit' => true, 'df_id' => $df_id)),
			'U_TODO_DELETE'	=> $this->helper->route('dl_ext_controller', array('view' => 'todo', 'action' => 'edit', 'delete' => true, 'submit' => true, 'df_id' => $df_id)),
		));
	}
}
else
