...
if($action == 'edit' || $action == 'add')
{
	...
	if($action == 'edit' && $cat_id && !$submit)
	{
		...
		$perms_copy_from	.= \oxpus\dl_ext\includes\classes\ dl_extra::dl_dropdown(0, 0, $index[$cat_id]['parent'], 'auth_view', $cat_id);
	...
	$cat_remain_traffic	= \oxpus\dl_ext\includes\classes\ dl_format::dl_size($cat_remain_traffic);
...
else if($action == 'delete' && $cat_id && !\oxpus\dl_ext\includes\classes\ dl_main::get_sublevel_count($cat_id))
{
	if ($confirm)
	{
		if (!check_form_key('dl_adm_cats'))
	...
	else
	{
		...
		$s_switch_cat .= \oxpus\dl_ext\includes\classes\ dl_extra::dl_dropdown(0, 0, $cat_id, 'auth_move');
		$s_switch_cat .= '</select>';
