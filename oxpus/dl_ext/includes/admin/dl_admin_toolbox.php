...
if ($action == 'browse' || $action == '' || $action == 'unassigned')
{
	if ($action != 'unassigned')
	{
		...
		$existing_files = \oxpus\dl_ext\includes\classes\ dl_physical::read_exist_files();
	...
	if ($files)
	{
		...
		$overall_size_tmp = \oxpus\dl_ext\includes\classes\ dl_format::dl_size($overall_size, 2, 'no');
		...
		else
		{
			...
			$s_file_action .= '<option value="del">'.$language->lang('DL_DELETE').'</option>';
			$s_file_action .= '<option value="---">---------------</option>';
			$s_file_action .= \oxpus\dl_ext\includes\classes\ dl_extra::dl_dropdown(0, 0, 0, 'auth_view');
		}
