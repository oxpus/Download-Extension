...
if ($submit && $check_user)
{
    ...
    if ($row)
    {
        ...
        $cat_perm_ary   = array();
        $dl_index       = array();
        $dl_index       = \oxpus\dl_ext\includes\classes\ dl_main::full_index();

        foreach ($dl_index as $cat_id => $value)
        {
        	$cat_perm_ary[$cat_id]['cat_name']		= $dl_index[$cat_id]['cat_name'];
        	$cat_perm_ary[$cat_id]['auth_view']		= \oxpus\dl_ext\includes\classes\ dl_auth::user_auth($cat_id, 'auth_view');
        	$cat_perm_ary[$cat_id]['auth_dl']		= \oxpus\dl_ext\includes\classes\ dl_auth::user_auth($cat_id, 'auth_dl');
        	$cat_perm_ary[$cat_id]['auth_up']		= \oxpus\dl_ext\includes\classes\ dl_auth::user_auth($cat_id, 'auth_up');
        	$cat_perm_ary[$cat_id]['auth_mod']		= \oxpus\dl_ext\includes\classes\ dl_auth::user_auth($cat_id, 'auth_mod');
        	$cat_perm_ary[$cat_id]['comment_read']	= \oxpus\dl_ext\includes\classes\ dl_auth::cat_auth_comment_read($cat_id);
        	$cat_perm_ary[$cat_id]['comment_post']	= \oxpus\dl_ext\includes\classes\ dl_auth::cat_auth_comment_post($cat_id);
        ...
        $template->assign_vars(array(
            'USER_IS_ADMIN'         => \oxpus\dl_ext\includes\classes\ dl_auth::user_admin(),
            'USER_IS_BANNED'        => \oxpus\dl_ext\includes\classes\ dl_auth::user_banned(),
            'USER_CAN_VIEW_STATS'   => \oxpus\dl_ext\includes\classes\ dl_auth::stats_perm(),
            'USER_CAN_SEE_TRACKER'  => \oxpus\dl_ext\includes\classes\ dl_auth::bug_tracker(),
            'USER_HAVE_TRAFFIC'     => \oxpus\dl_ext\includes\classes\ dl_format::dl_size($user->data['user_traffic']),
