...
foreach ($display_vars['vars'] as $config_name => $null)
{
	...
	if ($submit)
	{
		...
		{
			$this->new_config[$config_name] = $config_value = \oxpus\dl_ext\includes\classes\ dl_format::resize_value($config_name, $config_value);
		}
...
function select_rss_cats($value)
{
	...
	if ($value <> 0)
	{
		$rss_cats = \oxpus\dl_ext\includes\classes\ dl_extra::dl_cat_select(0, 0, array_map('intval', explode(',', $config['dl_rss_cats_select'])));
...
function select_size($value, $field, $size, $maxlength, $quote, $max_quote, $remain = false, $ext_path)
{
	...
	$quota_tmp = \oxpus\dl_ext\includes\classes\ dl_format::dl_size($config[$field], 2, 'select');
	...
	if ($remain)
	{
		switch ($field)
		{
			case 'dl_overall_traffic':
				...
				$remain_traffic_tmp = \oxpus\dl_ext\includes\classes\ dl_format::dl_size($remain_traffic, 2, 'none');
			...
			case 'dl_physical_quota':
				$remain_text_out = $language->lang('DL_PHYSICAL_QUOTA_EXPLAIN', \oxpus\dl_ext\includes\classes\ dl_format::dl_size(\oxpus\dl_ext\includes\classes\ dl_physical::read_dl_sizes(), 2));
			break;
		}
